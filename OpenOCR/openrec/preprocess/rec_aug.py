...
class UnionSTAugPIL(object):

    def __init__(self, **kwargs):
        self.transforms = rand_augment_transform()
        self.compression = A.ImageCompression(quality_lower=30, quality_upper=70, p=0.2)
        self.interpolations = [
            Image.BILINEAR,
            Image.BICUBIC,
            Image.LANCZOS,
    ...
    def __call__(self, data):
        img = data['image']
        interp_method = random.choice(self.interpolations)
        w, h = img.size
        scale = random.uniform(0.1, 1.0)
        new_w = max(1, int(w * scale))
        new_h = max(1, int(h * scale))
        img = img.resize((new_w, new_h), interp_method)
        
        img_np = np.array(img)
        augmented = self.compression(image=img_np)
